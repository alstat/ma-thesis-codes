...
surah = 1
bw_texts = verses(crps_tbl[surah])
...
# texts = map(x -> string.(x), split.(bw_texts))
# function debug_code()
#     r(texts[6][1], isarabic=false, first_word=false, silent_last_vowel=false)
# end

# Debugger.@enter debug_code()


k = 1
axs = []
for phases in rhythms
    push!(axs, Axis(grd1[k, 1], 
        title= k == 1 ? "Surah Al-Fatihah" : "",
        xlabel=k < length(rhythms) ? "" : "Time in Seconds",
        ylabel="Q$(surah):$(k)", 
        ygridvisible=true, 
        xgridvisible=false,
    ...
    steps = []; j = 2
    for phase in phases
        steps = vcat(steps, repeat([(j-1) % 2], phase))
        j += 1
    end
...
axs = []
for phases in rhythms
    push!(axs, Axis(grd2[k, 1], 
        title= k == 1 ? "Surah Al-Fiyl" :  "",
        xlabel=k < length(rhythms) ? "" : "Time in Seconds",
        ylabel="Q$(surah):$(k)", 
        ygridvisible=true, 
        xgridvisible=false,
...
axs = []
for phases in rhythms
    push!(axs, Axis(grd3[k, 1], 
        title= k == 1 ? "Surah Al-Kawthar" : "",
        xlabel=k < length(rhythms) ? "" : "Time in Seconds",
        ylabel="Q$(surah):$(k)", 
        ygridvisible=true, 
        xgridvisible=false,
...
axs = []
for phases in rhythms
    push!(axs, Axis(grd4[k, 1], 
        title=k == 1 ? "Surah Al-Ikhlas" : "",
        xlabel=k < length(rhythms) ? "" : "Time in Seconds",
        ylabel="Q$(surah):$(k)", 
        ygridvisible=true, 
        xgridvisible=false,
...
# save("plots/rhythmic_resultant.pdf", fig)


##### Ayah Resultant from its recitation, pos (noun), pos (verb), pos (adjective), pos (particles)
using Yunir
mfeat1 = parse.(QuranFeatures, crps_tbl[1][1].data.features)

isfeat(mfeat1[1], Prefix)
isfeat(mfeat1[2], Noun)
isfeat(mfeat1[5], Adjective)

mfeat2 = parse.(QuranFeatures, crps_tbl[1][6].data.features)
isfeat(mfeat2[1], Verb)
