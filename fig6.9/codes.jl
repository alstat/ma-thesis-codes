...
findall(x -> length(x) == 15, out)

# all_segments[295]
arabic(verses(crps_tbl)[199])
arabic(verses(crps_tbl)[295])
# Heatmap for 3 -> 11
...
for j in 1:3
    ...
    for i in 1:3
        ...
        if j == 3 && i == 3
            custom_colors = colors[[5,6,2]]
            custom_labels = ["Short Vowel", "Long Vowel", "Maddah"]

            # Create color boxes for the legend
            elements = [MarkerElement(color=color, marker=:rect, 
                                    strokecolor=:black, 
                                    strokewidth=1,
                                    markersize=20) for color in custom_colors]

            # Create the legend
            
            leg = Legend(grd[3,3], elements, custom_labels, "Color Legend",
                    orientation = :vertical, position=(-0.5,0.5)
                )
