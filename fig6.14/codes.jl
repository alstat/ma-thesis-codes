...
out = gen_rhythms(bw_texts)
get_transition_matrix(vcat(out...))

# cluster analysis
using Clustering
using Random
Random.seed!(123)
result = kmeans(collect(vec_dmatrix'), 20)
dump(result)

using StatsBase
countmap(result.assignments) |> print

# cluster 1

fig = Figure(size=(900,900))
grd = GridLayout(fig[1,1])
xlab = collect(1:114)[result.assignments .== 1, :]
ax1 = Axis(grd[1, 1], 
    ylabel = "States",
    title = "Cluster 1", 
    xminorticksvisible = true,
    yminorticksvisible = true,
    xticks = (collect(1:4:length(xlab)), string.(vec(xlab))[1:4:end]),
...
xlab = collect(1:114)[result.assignments .== 2, :]
ax2 = Axis(grd[1, 2], 
    ylabel = "States",
    title = "Cluster 2", 
    xminorticksvisible = true,
    yminorticksvisible = true,
    xticks = (collect(1:length(xlab)), string.(vec(xlab))[1:end]),
    aspect = AxisAspect(1.),
    xticklabelrotation = pi/2,
    yticks = (1:size(vec_dmatrix)[2], [
        "Short → Short", "Long → Short", "Maddah → Short",
...
xlab = collect(1:114)[result.assignments .== 3, :]
ax3 = Axis(grd[2, 1], 
    xlabel = "Surahs", 
    ylabel = "States", 
    title = "Cluster 3",
    xminorticksvisible = true,
    yminorticksvisible = true,
    xticks = (collect(1:length(xlab)), string.(vec(xlab))),
    aspect = AxisAspect(1.),
    # xticklabelrotation = pi/2,
    yticks = (1:size(vec_dmatrix)[2], [
        "Short → Short", "Long → Short", "Maddah → Short",
...
xlab = collect(1:114)[result.assignments .== 4, :]
ax4 = Axis(grd[2, 2], 
    xlabel = "Surahs", 
    ylabel = "States", 
    title = "Cluster 4",
...
hm1 = heatmap!(ax1,
    vec_dmatrix[result.assignments .== 1, :], 
    colormap=colors[[5,6,2]], 
    colorrange=(0,1)
)

heatmap!(ax2,
    vec_dmatrix[result.assignments .== 2, :], 
    colormap=colors[[5,6,2]], 
    colorrange=(0,1)
)

heatmap!(ax3,
    vec_dmatrix[result.assignments .== 3, :], 
    colormap=colors[[5,6,2]], 
    colorrange=(0,1)
)

heatmap!(ax4,
    vec_dmatrix[result.assignments .== 4, :], 
    colormap=colors[[5,6,2]], 
    colorrange=(0,1)
)
...
fig
# save("plots/rhythmic_trans_clusters.pdf", fig)